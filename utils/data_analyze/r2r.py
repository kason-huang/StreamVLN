#!/usr/bin/env python3
"""
R2R Dataset Analysis Script
Analyzes the structure and content of R2R (Room-to-Room) vision-and-language navigation dataset.

Usage:
    python utils/data_analyze/r2r.py
    python utils/data_analyze/r2r.py --data_path data/datasets/r2r/train/train.json.gz
    python utils/data_analyze/r2r.py --episode_id 5 --show_tokens
"""
...
def load_r2r_dataset(data_path: str) -> Dict[str, Any]:
    """Load R2R dataset from gzipped JSON file."""
...
def analyze_dataset_structure(data: Dict[str, Any]) -> None:
    ...
    print("=" * 80)
    print("R2R DATASET STRUCTURE ANALYSIS")
    print("=" * 80)
...
def print_complete_example(data: Dict[str, Any], episode_id: int = 0, show_tokens: bool = False) -> None:
    ...
    print("=" * 80)
    print(f"COMPLETE R2R DATASET EXAMPLE - Episode {episode_id}")
    print("=" * 80)
    ...
    if show_tokens:
        ...
        idx_to_word = {idx: word for word, idx in vocab['word2idx_dict'].items()}
        decoded_tokens = [idx_to_word.get(idx, '<unk>') for idx in tokens]
...
def analyze_instructions(episodes: List[Dict[str, Any]], sample_size: int = 1000) -> None:
    ...
    instruction_lengths = [len(instr.split()) for instr in instructions]

    print(f"  Word Count - Min: {min(instruction_lengths)}, Max: {max(instruction_lengths)}, Avg: {sum(instruction_lengths)/len(instruction_lengths):.1f}")
    ...
    direction_words = ['left', 'right', 'forward', 'straight', 'turn', 'go', 'stop', 'continue']
    direction_counts = {word: word_counts.get(word, 0) for word in direction_words}
    print(f"  Direction Words: {direction_counts}")


def main():
    parser = argparse.ArgumentParser(description='Analyze R2R dataset structure')
    parser.add_argument('--data_path', type=str, default='data/datasets/r2r/train/train.json.gz',
                       help='Path to R2R dataset file')
    ...
    try:
        ...
        data = load_r2r_dataset(args.data_path)
